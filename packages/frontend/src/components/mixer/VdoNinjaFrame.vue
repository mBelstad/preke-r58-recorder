...
<template>
  <div class="vdo-frame flex flex-col h-full bg-r58-bg-primary">
    <!-- Toolbar (App Chrome) -->
    <div 
      v-if="showToolbar !== false"
      class="vdo-frame__toolbar flex items-center justify-between px-4 py-2 bg-r58-bg-secondary border-b border-r58-bg-tertiary"
    >
      <!-- Left: Title and status -->
      <div class="flex items-center gap-3">
        <h3 v-if="title" class="text-sm font-semibold text-r58-text-primary">
          {{ title }}
        </h3>
        ...
        <div 
          ...
          class="flex items-center gap-1.5 px-2 py-1 bg-r58-bg-tertiary rounded text-xs"
        >
    ...
    <div class="vdo-frame__content flex-1 relative min-h-0">
      ...
      <div 
        v-if="lastError"
        class="absolute bottom-4 left-4 right-4 bg-r58-accent-danger/90 text-white px-4 py-3 rounded-lg shadow-lg flex items-start gap-3"
      >
    ...
    <div v-if="$slots.bottom" class="vdo-frame__bottom border-t border-r58-bg-tertiary">
