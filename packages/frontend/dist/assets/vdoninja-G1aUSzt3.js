...
import{N as h}from"./index-C-UnMYRj.js";let s=null;async function i(){if(s)return s;try{const{getDeviceUrl:o}=await h(async()=>{const{getDeviceUrl:r}=await import("./index-C-UnMYRj.js").then(e=>e.a3);return{getDeviceUrl:r}},__vite__mapDeps([0,1])),a=o();if(a){const r=new AbortController,e=setTimeout(()=>r.abort(),3e3),n=await fetch(`${a}/api/config`,{signal:r.signal,mode:"cors",cache:"no-cache"});if(clearTimeout(e),n.ok){const c=await n.json();if(c.vdo_host||c.vdoninja_host)return s=c.vdo_host||c.vdoninja_host,s}}}catch{console.log("[VDO.ninja] Device does not provide VDO.ninja host configuration")}const t="r58-vdo.itagenten.no";return s=t,console.log(`[VDO.ninja] Using default host: ${t}`),t}function u(){return"https"}const l="studio",b="preke-r58-2024";function d(){var o,a;const t=`
